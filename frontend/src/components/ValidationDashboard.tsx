...
const ValidationDashboard: React.FC = () => {
  ...
  const [selectedSubmission, setSelectedSubmission] =
    useState<RitualSubmission | null>(null);
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead
                onClick={() => handleSort('esepScore')}
                className='cursor-pointer'
              >
                ESEP Score{' '}
                {sortConfig.key === 'esepScore' &&
                  (sortConfig.direction === 'asc' ? (
                    <ChevronUp size={16} />
                  ) : (
                    <ChevronDown size={16} />
              ...
              <TableHead
                onClick={() => handleSort('cedaScore')}
                className='cursor-pointer'
              >
                CEDA Score{' '}
                {sortConfig.key === 'cedaScore' &&
                  (sortConfig.direction === 'asc' ? (
                    <ChevronUp size={16} />
                  ) : (
                    <ChevronDown size={16} />
        ...
        >
          ...
          >
            ...
            <div className='space-y-2'>
              <p>
                <strong>Submitter:</strong> {selectedSubmission.submitter}
              </p>
              <p>
                <strong>ESEP Score:</strong> {selectedSubmission.esepScore}
              </p>
              <p>
                <strong>CEDA Score:</strong> {selectedSubmission.cedaScore}
              </p>
              <p>
                <strong>Status:</strong> {selectedSubmission.approvalStatus}
              </p>
              <p>
                <strong>IPFS Hash:</strong>{' '}
                <a
                  href={`https://ipfs.io/ipfs/${selectedSubmission.ipfsHash}`}
                  target='_blank'
                  className='text-blue-500 hover:underline'
                >
                  {selectedSubmission.ipfsHash.slice(0, 8)}...
                </a>
              </p>
              <p>
                <strong>Transaction:</strong>{' '}
                {selectedSubmission.txHash === 'Pending' ? (
                  'Pending'
                ) : (
                  <a
                    href={`https://etherscan.io/tx/${selectedSubmission.txHash}`}
                    target='_blank'
                    className='text-blue-500 hover:underline'
                  >
                    {selectedSubmission.txHash.slice(0, 8)}...
                  </a>
                )}
              </p>
            </div>
